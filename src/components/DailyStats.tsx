...
function getBackground(stat: any, problems: Boulder[], date: Date): string {
  if (
    filterBySetDay(problems, date).filter(
      (p) => p.difficulty === stat.difficultyValue
    ).length > 0
  ) {
...
export const DailyStats = ({ sentProblems }: DailyStatsProps) => {
  ...
  const stats = data.map((stat) => (
    <Paper
      style={{ backgroundColor: getBackground(stat, sentProblems, date) }}
      ...
      <div>
        ...
        <Text size="xs" className={classes.count}>
          <span className={classes.value}>
            {
              filterBySetDay(sentProblems, date).filter(
                (p) => p.difficulty === stat.difficultyValue
              ).length
            }{" "}
            problems
